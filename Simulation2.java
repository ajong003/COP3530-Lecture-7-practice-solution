...
public class Simulation2 {
    ...
    public static void main(String[] args) {
        ...
        for (int minute = 1; minute <= SIMULATION_TIME; minute++) {
            ...
            if (customerArrives(ARRIVAL_PROBABILITY)) {
                emptiestQueue(queues, registerAvailableTimes,minute).add(new Customer(minute));
            }
            for (int i = 0; i < queues.length; i++) {
                ...
                if (!queues[i].isEmpty() && registerAvailableTimes[i] <= minute) {
                    numCustomers += 1;
                    System.out.println("added " + (minute - queues[i].peek().getTimeEnteredQueue()));
                    totalWaitTime += minute - queues[i].peek().getTimeEnteredQueue();
                    registerAvailableTimes[i] = minute + queues[i].peek().getTimeNeededAtRegister();
                    queues[i].remove();
                }
            }
    ...
    private static Queue<Customer> emptiestQueue(Queue<Customer>[] queues, int[] timeleft, int minute) {

        int[] waittimes = {timeleft[0] - minute, timeleft[1] - minute, timeleft[2] - minute};

        int[] queuesizes = {queues[0].size(), queues[1].size(), queues[2].size()};
        ...
        int mind=0;
        if (queues[0].size() == queues[1].size() && queues[1].size() == queues[2].size()) {

            for (int i = 0; i < 3; i++) {
                if (waittimes[i] < waittimes[mind]) {
                    mind = i;
                }
            }
            return queues[mind];
        } else if (queues[0].size() == queues[1].size() && queues[0].size()==min) {
        ...
        else if (queues[0].size() == queues[2].size()&&queues[0].size()==min) {
        ...
        else if (queues[1].size() == queues[2].size() && queues[1].size()==min) {
